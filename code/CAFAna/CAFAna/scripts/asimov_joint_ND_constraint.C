...
#include "CAFAna/Systs/SystPriorCovar.h"
...
void SetOscillationParameter(osc::IOscCalculatorAdjustable *calc,
  ...
  else if (parName == "dmsq32NHscaled")
...
// Acceptable parameter names: th13, ss2th13, delta(pi), th23, ssth23, ss2th23, dmsq32, dmsq32NHscaled, tsth12, ss2th12, dmsq21, rho
void asimov_joint_ND_constraint(std::string stateFname="common_state_mcc11v3_broken.root",
  ...
  osc::IOscCalculatorAdjustable* trueOsc = NuFitOscCalc(hie, 1, std::stoi(asimov_set));
  ...
  osc::IOscCalculatorAdjustable* testOsc = NuFitOscCalc(hie, 1, std::stoi(asimov_set));
  
  MultiExperiment MultiPenalty_nom;
  IExperiment *penalty_nom = GetPenalty(hie, 1, penaltyString, asimov_set);
  MultiPenalty_nom.Add(penalty_nom);
  if (ndPostFit.size()){

    std::cout << "Getting file" << std::endl;
    TFile * fPrior = new TFile(ndPostFit.c_str());
    std::cout << "Got File, Getting TTree." << std::endl;
    TTree * fit_info = (TTree*) fPrior->Get("fit_info");
    std::cout << "Got TTree, Getting TH2." << std::endl;
    TH2D * nd_covar = (TH2D*) fPrior->Get("covar");
    std::cout << "Got TH2. Declaring vectors and setting branch addresses..." << std::endl;
    
    std::vector<double> * priorValues = new std::vector<double>();
    std::vector<std::string> * paramNames = new std::vector<std::string>();
    
    // Get Central values and parameter names
    fit_info->SetBranchAddress("fPostFitValues", &priorValues);
    fit_info->SetBranchAddress("fParamNames", &paramNames);
    std::cout << "Addresses declared, getting entry 0" << std::endl;
    fit_info->GetEntry(0);
    std::cout << "Got entry 0" << std::endl;

    
    TVectorD priorVals = TVectorD(priorValues->size(), &(*priorValues)[0]);
    
    TMatrixD priorCovMat(nd_covar->GetNbinsX(),nd_covar->GetNbinsY());
    for (int i = 0; i < nd_covar->GetNbinsX(); i++){
      for(int j = 0; j < nd_covar->GetNbinsY(); j++){
	priorCovMat[i][j] = nd_covar->GetBinContent(i+1, j+1);
      }
    }
    
    std::cout << "Number of prior values " << priorVals.GetNrows() << std::endl;
    std::cout << "cov mat dimentions " << priorCovMat.GetNrows() << " " << priorCovMat.GetNcols() << std::endl;
    priorCovMat.Print();

    std::cout << "Got priors from file" << std::endl;
    SystPriorCovar * ndPostFitPenalty = new SystPriorCovar(paramNames, priorVals, priorCovMat, systlist);
    std::cout << "Got SystPriorCovar" << std::endl;

    MultiPenalty_nom.Add(ndPostFitPenalty);
  ...
  for (int xBin = 0; xBin < sens_hist->GetNbinsX(); ++xBin){
    ...
    for (int yBin = yMin; yBin < yMax; ++yBin){
      ...
      if ((firstPoint >= 0) && ((pointCounter) < firstPoint)) continue;
      if ((lastPoint >= 0) && ((pointCounter) >= lastPoint)) continue;

      std::cout << "Running point " << pointCounter << std::endl;
	...
	SetOscillationParameter(testOsc, plotVarVect[1], yCenter, hie);
      ...
      std::cout << "dmsq32 " << testOsc->GetDmsq32() << std::endl;
      std::cout << "th23 " << testOsc->GetTh23() << std::endl;

      IExperiment *penalty = GetPenalty(hie, 1, penaltyString, asimov_set);
      MultiExperiment MultiPenalty;
	...
	// FIRST! Assume order of parameters is [osc, syst] and find index of first syst parameter
	...
	MultiPenalty.Add(ndPostFitPenalty);
