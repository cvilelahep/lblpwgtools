...
// Scan over true delta and find best fit delta for each point
void delta_delta_joint(std::string stateFname="common_state_mcc11v3_broken.root",
  ...
  for (double testDeltaPi = 0.; testDeltaPi < 2.; testDeltaPi += 0.05){
    sprintf(dirNameBuff, "trueDeltaPiTest_%i", count++);
